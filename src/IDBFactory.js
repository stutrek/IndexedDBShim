...
function triggerAnyVersionChangeAndBlockedEvents(
    openConnections,
    req,
    oldVersion,
    newVersion
    ...
    const connectionsClosed = () =>
        openConnections.every(conn => {
            return connectionIsClosed(conn);
        });
    return openConnections
        .reduce(function(promises, entry) {
...
function getLatestCachedWebSQLVersion(name) {
    return Object.keys(websqlDBCache[name])
        .map(version => {
...
function getLatestCachedWebSQLDB(name) {
    return (
        websqlDBCache[name] &&
        websqlDBCache[name][getLatestCachedWebSQLVersion(name)]
    );
}

function cleanupDatabaseResources(
    __openDatabase,
    name,
    escapedDatabaseName,
    databaseDeleted,
    ...
    if (useMemoryDatabase) {
        const latestSQLiteDBCached = websqlDBCache[name]
            ? getLatestCachedWebSQLDB(name)
            : null;
        if (!latestSQLiteDBCached) {
            console.warn(
                'Could not find a memory database instance to delete.'
            );
            databaseDeleted();
            return;
        }
        const sqliteDB =
            latestSQLiteDBCached._db && latestSQLiteDBCached._db._db;
        if (!sqliteDB || !sqliteDB.close) {
            console.error(
                'The `openDatabase` implementation does not have the expected `._db._db.close` method for closing the database'
            );
            return;
        }
    ...
    if (CFG.fs && CFG.deleteDatabaseFiles !== false) {
        CFG.fs.unlink(
            CFG.pathJoin(CFG.databaseBasePath || '', escapedDatabaseName),
            err => {
            ...
            function(tx, data) {
                ...
                (function deleteTables(i) {
                    if (i >= tables.length) {
                        ...
                        tx.executeSql(
                            'DROP TABLE ' +
                                util.escapeStoreNameForSQL(
                                    util.unescapeSQLiteResponse(
...
function createSysDB(__openDatabase, success, failure) {
    ...
    if (sysdb) {
        ...
        sysdb = __openDatabase(
            typeof CFG.memoryDatabase === 'string'
                ...
                : CFG.pathJoin(
                      ...
                      '__sysdb__' +
                          (CFG.addSQLiteExtension !== false ? '.sqlite' : '')
                  ),
            1,
            'System Database',
            CFG.DEFAULT_DB_SIZE
        ...
        sysdb.transaction(function(systx) {
            systx.executeSql(
                'CREATE TABLE IF NOT EXISTS dbVersions (name BLOB, version INT);',
                [],
                function(systx) {
...
IDBFactory.prototype.open = function(name /* , version */) {
    ...
    if (hasNullOrigin()) {
        throw createDOMException(
            'SecurityError',
            'Cannot open an IndexedDB database from an opaque origin.'
        );
    }
    ...
    const useDatabaseCache =
        CFG.cacheDatabaseInstances !== false || useMemoryDatabase;
    ...
    function setupDatabase(tx, db, oldVersion) {
        tx.executeSql(
            'SELECT "name", "keyPath", "autoInc", "indexList" FROM __sys__',
            [],
            function(tx, data) {
                ...
                const connection = IDBDatabase.__createInstance(
                    db,
                    name,
                    oldVersion,
                    version,
                ...
                if (oldVersion < version) {
                    ...
                    triggerAnyVersionChangeAndBlockedEvents(
                        openConnections,
                        req,
                        oldVersion,
                        version
                        ...
                        .then(function() {
                                    ...
                                    try {
                                        ...
                                        sysdb.transaction(function(systx) {
                                            function reportError(msg) {
                                                throw new Error(
                                                    'Unable to roll back upgrade transaction!' +
                                                        (msg || '')
                                                );
                                            }
                            ...
                            sysdb.transaction(
                                function(systx) {
                                    function versionSet() {
                                        const e = new IDBVersionChangeEvent(
                                            'upgradeneeded',
                                            { oldVersion, newVersion: version }
                                        );
                                        req.__result = connection;
                                        connection.__upgradeTransaction = req.__transaction = req.__result.__versionTransaction = IDBTransaction.__createInstance(
                                            req.__result,
                                            req.__result.objectStoreNames,
                                            'versionchange'
                                        );
                                        req.__done = true;
                                        ...
                                        req.transaction.__addNonRequestToTransactionQueue(
                                            function onupgradeneeded(
                                                ...
                                                if (
                                                    e.__legacyOutputDidListenersThrowError
                                                ) {
                                                    logError(
                                                        'Error',
                                                        'An error occurred in an upgradeneeded handler attached to request chain',
                                                        e.__legacyOutputDidListenersThrowError
                                                    ); // We do nothing else with this error as per spec
                                                    req.transaction.__abortTransaction(
                                                        createDOMException(
                                                            'AbortError',
                                        ...
                                        req.transaction.on__beforecomplete = function(
                                            ...
                                            sysdbFinishedCb(
                                                ...
                                                false,
                                                function() {
                                                    req.transaction.__transFinishedCb(
                                                        false,
                                                        function() {
                                                            ev.complete();
                        ...
                        .catch(err => {
                            console.log(
                                'Error within `triggerAnyVersionChangeAndBlockedEvents`'
                            );
                            throw err;
                        });
                } else {
    ...
    function openDB(oldVersion) {
        let db;
        if (
            (useMemoryDatabase || useDatabaseCache) &&
            name in websqlDBCache &&
            websqlDBCache[name][version]
        ) {
        ...
        db.transaction(function(tx) {
            tx.executeSql(
                'CREATE TABLE IF NOT EXISTS __sys__ (name BLOB, keyPath BLOB, autoInc BOOLEAN, indexList BLOB, currNum INTEGER)',
                [],
                function() {
                    function setup() {
                    ...
                    tx.executeSql(
                        'CREATE INDEX IF NOT EXISTS sysname ON __sys__(name)',
                        [],
                        setup,
                        dbCreateError
                    );
            ...
            createSysDB(
                ...
                function() {
                    sysdb.readTransaction(function(sysReadTx) {
                        sysReadTx.executeSql(
                            'SELECT "version" FROM dbVersions WHERE "name" = ?',
                            [sqlSafeName],
                            function(sysReadTx, data) {
...
IDBFactory.prototype.deleteDatabase = function(name) {
    ...
    if (hasNullOrigin()) {
        throw createDOMException(
            'SecurityError',
            'Cannot delete an IndexedDB database from an opaque origin.'
        );
    }
                            ...
                            triggerAnyVersionChangeAndBlockedEvents(
                                ...
                                sysdb.transaction(
                                    ...
                                    function(
                                        ...
                                        sysdbFinishedCbDelete = function(
                                            err,
                                            cb
                                        ) {
                                            if (err) {
...
IDBFactory.prototype.databases = function() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (hasNullOrigin()) {
            throw createDOMException(
                'SecurityError',
                'Cannot get IndexedDB database names from an opaque origin.'
            );
        }
...
IDBFactory.prototype.__forceClose = function(dbName, connIdx, msg) {
    ...
    if (util.isNullish(dbName)) {
        ...
        !Number.isInteger(connIdx) ||
        connIdx < 0 ||
        connIdx > me.__connections[dbName].length - 1
    ) {
        throw new TypeError(
